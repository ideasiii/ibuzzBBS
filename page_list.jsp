...
JSONArray itemJSONArr = srv.getSettings(dbname,fieldVector,conditionFieldVector,conditionFieldValueVector,0,0,"CONVERT(page_category using big5),CONVERT(page_name using big5)","asc");

String page_ids =  request.getParameter("page_ids").toString();
%>
...
<script type="text/javascript">
	function loadFBPage(page_ids){
	    var a = page_ids.split(",") // Delimiter is a string
			for (var i = 0; i < a.length; i++)
			{
				var el = document.getElementsByName('page_id');
		    var len = el.length;
		    for(var j=0; j<len; j++)
		    {
		        if(el[j].value == a[i]&&a[i]!='')
		        {
		           el[j].checked = true ;
		        }
		    }
	...
	function addFBPage(){
	    ...
	    for(var i=0; i<len; i++)
	    {
	        ...
	        {
	           page_ids = page_ids + el[i].value + ","; 
	           page_accounts = page_accounts + el[i].id + ","; 
	        }
	    }
	    opener.document.getElementById('page_ids').value=page_ids.substring(0,page_ids.length-1);
	    opener.document.getElementById('page_accounts').value=page_accounts.substring(0,page_accounts.length-1);
	    window.close();
	}
	
	function clearFBPage(){
	    var el = document.getElementsByName('page_id');
	    var len = el.length;
	    for(var i=0; i<len; i++)
	    {
	...
	function selectCategoryFBPage(category_num){
      ...
      var page_input = category_div.getElementsByTagName("input");
      for(var i=0; i<page_input.length; i++){
          if(document.getElementById(category_num).checked == true){
             page_input[i].checked = true;
          }else{
          	 page_input[i].checked = false;
          }
      }
	}
</script>
...
<body onLoad="loadFBPage('<%=page_ids%>');">
<div id="wrapper" class="clear">
    <div id="main" style="width:700px">
	<div >
		<h2>FB擷取粉絲團/社群/個人列表</h2><br/>
		...
		for(int i=0;i<itemJSONArr.length();i++){
		    ...
		    if(!itemJSON.getString("page_category").equals(category)){
		%>
		    </div>
		    <%j=j+1;%>
		    <br/><br/><li><font size="4"><input type="checkbox" name="<%=j%>" id="<%=j%>" value="<%=itemJSON.getString("page_category")%>" onclick="selectCategoryFBPage('<%=j%>');"><%=itemJSON.getString("page_category")%></input></></font></li>
		    <div id="category_<%=j%>">
		<%  }
		%>		
		    <input type="checkbox" name="page_id" id="<%=itemJSON.getString("page_name").replace(",","").replace("'","").replace("\"","")%>" value="<%=itemJSON.getString("page_id")%>"><%=itemJSON.getString("page_name").replace(",","").replace("'","").replace("\"","")%></input>&nbsp;
		<%
		    category = itemJSON.getString("page_category");
		}%>		
		</ul>
	  <input type="button" value="送出" class="btnposition" onClick="addFBPage();">
	  <input type="button" value="全部清除" class="btnposition" onClick="clearFBPage();">
